# google-ads-mcp-extended (BetterMe)

Розширення офіційного [googleads/google-ads-mcp](https://github.com/googleads/google-ads-mcp):
до read-only інструментів додані **write-інструменти** для керування кампаніями.
...
Рекомендація: у Claude Desktop → Connectors → google-ads виставити
write-інструментам режим **Ask first**.
...
2. Авторизація (одноразово; потрібен client_secret.json OAuth-клієнта з
   Google Cloud Console проекту команди):
...
3. Блок у `claude_desktop_config.json` (Claude → Settings → Developer → Edit Config):
   ...
   "google-ads": {
     ...
     "args": [
       "-m", "pipx", "run", "--no-cache",
       "--spec", "git+https://github.com/BETTERME_ORG/google-ads-mcp-extended.git",
       "google-ads-mcp"
     ],
     "env": {
       "GOOGLE_APPLICATION_CREDENTIALS": "/Users/USERNAME/.config/gcloud/application_default_credentials.json",
       "GOOGLE_PROJECT_ID": "quantum-hash-501912-nh",
       "GOOGLE_ADS_DEVELOPER_TOKEN": "DEV_TOKEN_КОМАНДИ",
       "GOOGLE_ADS_LOGIN_CUSTOMER_ID": "2539412596"
     }
   }
   ```
...
4. Повний перезапуск Claude Desktop (Cmd+Q).

## Що змінено відносно офіційного репозиторію
...
- Developer token команди повинен мати Basic/Standard access (Standard є).
...
6. `ad_group.demand_gen_ad_group_settings.channel_controls` — режим (SELECTED_CHANNELS чи CHANNEL_STRATEGY) і конкретні канали. У шаблонах BetterMe: video-групи = YT in-stream+in-feed+Shorts; image-групи = ALL_OWNED_AND_OPERATED.
...
Перевірено на клоні US_ALL_pMAX_Feed2_All_Clothes&Equipment (store.betterme.world):
